...
class Metode_abs extends CI_Model {

	function get_sumber($tgl_awal, $tgl_akhir, $jurusan){
		$tahun = $tgl_awal;
		$tgl_awal = $tahun.'-01-01 00:00:00';
		$tgl_akhir = $tahun.'-12-31 23:59:00';
		return $this->db->query('select distinct(abstrak.namadokumen) as nama2, file_pdf.* from abstrak,file_pdf where file_pdf.jurusan = ? and file_pdf.nama = abstrak.namadokumen and file_pdf.tgl_up between ? and ?',array($jurusan, $tgl_awal, $tgl_akhir))->result();
	}
	...
	function get_target($tgl_akhir, $jurusan){
		...
		$tgl_akhir = $tahun.'-12-31 23:59:00';
		return $this->db->query('select distinct(abstrak.namadokumen) as nama2, file_pdf.* from abstrak,file_pdf where file_pdf.jurusan = ? and file_pdf.nama = abstrak.namadokumen and file_pdf.tgl_up between ? and ?',array($jurusan, $tgl_awal, $tgl_akhir))->result();
		#return $this->db->query('select distinct(abstrak.namadokumen) as nama2, file_pdf.* from abstrak,file_pdf where file_pdf.nama = abstrak.namadokumen and file_pdf.tgl_up > ?',array($tgl_akhir))->result();
	...
	function get_TiapKataSumber($pdf_sumber){
		...
		$_BanyakDocSumber = count($nama_File_Pdf_Sumber);
			...
			for ($nomor_DokumenSumber=0; $nomor_DokumenSumber < sizeof($nama_File_Pdf_Sumber) ; $nomor_DokumenSumber++) { 
				...
				$querySumber = $this->db->query('select *, kata as kata_sumber from abstrak where namadokumen =?',array($nama_File_Pdf_Sumber[$nomor_DokumenSumber]))->result();

				$queryDocSumber = $this->db->query('select *, kata as kata_sumber from abstrak where namadokumen =?',array($nama_File_Pdf_Sumber[$nomor_DokumenSumber]))->row();
	...
	function get_TiapKataTarget($pdf_target){
		...
		$_BanyakDocTarget = count($nama_File_Pdf_Target);
			...
			for ($nomor_DokumenTarget=0; $nomor_DokumenTarget < sizeof($nama_File_Pdf_Target) ; $nomor_DokumenTarget++) { 
				...
				$querytarget = $this->db->query('select *, kata as kata_target from abstrak where namadokumen =?',array($nama_File_Pdf_Target[$nomor_DokumenTarget]))->result();

				$queryDocTarget = $this->db->query('select *, kata as kata_target from abstrak where namadokumen =?',array($nama_File_Pdf_Target[$nomor_DokumenTarget]))->row();
...
/* End of file Metode_abs.php */
/* Location: ./application/models/Metode_abs.php */
